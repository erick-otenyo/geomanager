$((async function () {
    ...
    const fetchLayerFiles = (layerId) => {
        const rastersUrl = `${window.layermanager_opts.fileRasterApiBaseUrl}?layer=${layerId}`
    ...
    const fetchRasterBounds = (rasterId) => {
        const metadataUrl = `${window.layermanager_opts.fileRasterApiBaseUrl}/${rasterId}/info/metadata`
    ...
    const fetchColorMaps = () => {
        const largeImageApiBaseUrl = `${window.layermanager_opts.layerImageApiBaseUrl}/colormaps`
    ...
    const setLayer = async (layerId) => {
        ...
        const tilesUrl = updateTileUrl(window.layermanager_opts.layerTilesUrl, params)
