$((async function () {
    ...
    const fetchVectorTables = (layerId) => {
        const vectorsUrl = `${window.layermanager_opts.dataVectorApiBaseUrl}?layer=${layerId}`
    ...
    const setLayer = async (layerId) => {
        ...
        if (geometry_type && selectedVectorTable) {
            ...
            const tileUrl = updateTileUrl(window.layermanager_opts.vectorTilesUrl, url_params)
