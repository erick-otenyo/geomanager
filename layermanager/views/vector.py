...
from layermanager.forms import VectorLayerFileForm, BoundaryUploadForm
...
def upload_vector_file(request, dataset_id=None, layer_id=None):
    ...
    if request.method == 'POST':
        ...
        ctx = {
            "form": layer_form,
            "dataset": dataset,
            "publish_action": reverse("layermanager_publish_vector", args=[upload.pk]),
            "delete_action": reverse("layermanager_delete_vector_upload", args=[upload.pk]),
        }
    ...
    def get(self, request, z, x, y):
        ...
        sql = f"""WITH
            ...
            mvtgeom AS (
              SELECT ST_AsMVTGeom(ST_Transform(t.geom, 3857), bounds.geom) AS geom,
                *
              FROM layermanager_countryboundary t, bounds
