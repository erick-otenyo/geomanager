...
def get_layer_action_url(layer_type, action, action_args=None):
    if layer_type == "file":
        from layermanager.models import FileImageLayer
        ...
        from layermanager.models.vector import VectorLayer
...
def get_preview_url(layer_type, dataset_id, layer_id=None):
    ...
    if layer_type == "file":
        if layer_id:
            preview_url = reverse(
                f"layermanager_preview_raster_layer",
                args=args,
            )
        else:
            preview_url = reverse(
                f"layermanager_preview_raster_dataset",
                args=args,
            )
    elif layer_type == "vector":
        if layer_id:
            ...
            preview_url = reverse(
                f"layermanager_preview_vector_dataset",
                args=args,
            )
        ...
        if layer_id:
            preview_url = reverse(
                f"layermanager_preview_wms_layer",
                args=args,
            )
        else:
            preview_url = reverse(
                f"layermanager_preview_wms_dataset",
                args=args,
            )
...
def get_upload_url(layer_type, dataset_id, layer_id=None):
    ...
    if layer_type == "file":
        if layer_id:
            upload_url = reverse(
                f"layermanager_dataset_layer_upload_raster",
                args=args,
            )
        else:
            upload_url = reverse(
                f"layermanager_dataset_upload_raster",
                args=args,
            )
    elif layer_type == "vector":
        if layer_id:
            ...
            upload_url = reverse(
                f"layermanager_dataset_upload_vector",
                args=args,
            )
    else:
        upload_url = None
